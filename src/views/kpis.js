...
export async function renderKpis(el, me) {
  ...
  async function load() {
    ...
    } catch (error) { box.innerHTML = emptyHTML('triangleAlert', error.message); }
  }
...
async function renderEmployeeKpi(box, month, year, refresh) {
  try {
    ...
    if (!plan || !items.length) { box.innerHTML = emptyHTML('target', 'Bạn chưa có KPI trong tháng này'); return; }
...
async function detail(employeeId, month, year, me, refresh) {
  try {
    ...
    if (!plan) { if (hr(me)) return editor(me, refresh, employeeId, month, year); openModal('KPI tháng', emptyHTML('target', 'Chưa có KPI')); return; }
...
async function editor(me, refresh, presetId = '', presetMonth = now().month, presetYear = now().year) {
  ...
  function renderUsers() {
    ...
    pager.innerHTML = visible.length > pageSize ? `<button class="btn-secondary btn-sm kpi-page" data-page="${page - 1}" ${page === 1 ? 'disabled' : ''} style="display:inline-flex;align-items:center;gap:4px;">${icon('arrowLeft', 'xs')} <span>Trước</span></button><span>${(page - 1) * pageSize + 1}–${Math.min(page * pageSize, visible.length)} / ${visible.length}</span><button class="btn-secondary btn-sm kpi-page" data-page="${page + 1}" ${page === pages ? 'disabled' : ''} style="display:inline-flex;align-items:center;gap:4px;"><span>Sau</span> ${icon('arrowRight', 'xs')}</button>` : (visible.length ? `<span>Hiển thị ${visible.length} người</span>` : '');
