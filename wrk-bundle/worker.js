...
async function migrate(env) {
  ...
  try {
    await env.DB.exec("ALTER TABLE subtasks ADD COLUMN description TEXT");
  } catch (_) {
  }
  ...
  try {
    await env.DB.exec(`ALTER TABLE eval_periods ADD COLUMN hr_note_at TEXT`);
  } catch (_) {
  }
  try {
    ...
    if (row.checkout_time) {
      ...
      if (checkoutLateMinutes > 0) {
        notifications.push({
          ...
          title: "Checkout tr\u1EC5",
          message: `${row.full_name} checkout tr\u1EC5 ${checkoutLateMinutes} ph\xFAt l\xFAc ${row.checkout_time}${row.overtime_status ? `, OT ${row.overtime_status === "approved" ? "\u0111\xE3 duy\u1EC7t" : row.overtime_status === "pending" ? "\u0111ang ch\u1EDD duy\u1EC7t" : "\u0111\xE3 t\u1EEB ch\u1ED1i"}` : ""}`,
...
async function ensureSubtaskSchema(env) {
  await env.DB.exec(`CREATE TABLE IF NOT EXISTS subtasks (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    task_id INTEGER NOT NULL,
    title TEXT NOT NULL,
    description TEXT,
...
async function ensureAttendanceLocationSchema(env) {
  ...
  try {
    await env.DB.prepare(`INSERT INTO attendance_locations
      ...
      SELECT ?,?,?,?,?,?,?,1
       WHERE NOT EXISTS (SELECT 1 FROM attendance_locations WHERE code=?)`).bind("V\u0103n ph\xF2ng H\xE0 N\u1ED9i", "NETVIET-HN", "H\xE0 N\u1ED9i", 21.018472, 105.793595, 100, 100, "NETVIET-HN").run();
  } catch (error) {
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Worksheet ss:Name="Nh\xE2n vi\xEAn"><Table>
  ...
  if (path === "/api/users" && request.method === "POST") {
    ...
    if (!b.full_name || !b.email || !b.phone || !b.birth_date || !b.national_id || !b.home_address || !b.department || !b.position || !b.direct_manager_id || !b.work_location || !b.contract_type || !b.hire_date) {
      return json({ error: "Vui l\xF2ng nh\u1EADp \u0111\u1EA7y \u0111\u1EE7 c\xE1c tr\u01B0\u1EDDng b\u1EAFt bu\u1ED9c" }, 400);
    }
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      try {
        ...
        lastErr = e;
        if (e.message.includes("UNIQUE") && e.message.includes("employee_code")) continue;
        if (e.message.includes("UNIQUE")) return json({ error: "Email \u0111\xE3 t\u1ED3n t\u1EA1i" }, 400);
        throw e;
      }
    }
    console.error(lastErr);
    return json({ error: "Kh\xF4ng th\u1EC3 sinh m\xE3 nh\xE2n vi\xEAn, vui l\xF2ng th\u1EED l\u1EA1i" }, 500);
  }
  ...
  if (commentsMatch) {
    ...
    if (request.method === "POST") {
      ...
      const r = await env.DB.prepare("INSERT INTO task_comments (task_id,user_id,content) VALUES (?,?,?)").bind(tid, me.id, b.content).run();
      return json({ ok: true, id: r.meta.last_row_id });
    }
  }
  ...
  if (path === "/api/leave" && request.method === "POST") {
    try {
      ...
      await env.DB.prepare("INSERT INTO leave_approval_history (leave_request_id,approval_level,actor_id,actor_name,action,note) VALUES (?,?,?,?,?,?)").bind(r.meta.last_row_id, 0, me.id, me.full_name, "submitted", needsBod ? "Lu\u1ED3ng c\u1EA7n Ban Gi\xE1m \u0111\u1ED1c ph\xEA duy\u1EC7t cu\u1ED1i" : "Lu\u1ED3ng Qu\u1EA3n l\xFD tr\u1EF1c ti\u1EBFp \u2192 HCNS").run();
      return json({ ok: true, id: r.meta.last_row_id });
    } catch (e) {
      console.error("Leave create failed", e);
      return json({ error: "Kh\xF4ng th\u1EC3 t\u1EA1o y\xEAu c\u1EA7u ngh\u1EC9 ph\xE9p, vui l\xF2ng th\u1EED l\u1EA1i sau" }, 500);
    }
  }
  ...
  if (leaveDocumentMatch && request.method === "GET") {
    ...
    if (Number(document.employee_id) !== Number(me.id) && !canManageLeaveRequest(me, document)) return json({ error: "Kh\xF4ng c\xF3 quy\u1EC1n xem t\xE0i li\u1EC7u" }, 403);
    if (!env.HR_DOCUMENTS) return json({ error: "L\u01B0u tr\u1EEF t\xE0i li\u1EC7u ch\u01B0a \u0111\u01B0\u1EE3c c\u1EA5u h\xECnh" }, 503);
    const object = await env.HR_DOCUMENTS.get(document.storage_key);
    if (!object) return json({ error: "T\u1EC7p kh\xF4ng t\u1ED3n t\u1EA1i tr\xEAn kho l\u01B0u tr\u1EEF" }, 404);
    const disposition = url.searchParams.get("disposition") === "attachment" ? "attachment" : "inline";
  ...
  if (msgMatch && request.method === "DELETE") {
    const msgId = parseInt(msgMatch[1]);
    const now = (/* @__PURE__ */ new Date()).toISOString().replace("T", " ").slice(0, 19);
    await env.DB.prepare("UPDATE messages SET deleted_at = ? WHERE id = ? AND sender_id = ?").bind(now, msgId, me.id).run();
    return json({ ok: true });
  }
